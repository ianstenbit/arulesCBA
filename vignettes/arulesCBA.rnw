...
Note that the second parameter to the \code{CBA} function, \code{class}, is the name of the column of \code{iris.disc} which contains the class of each entry.

The association rules of the classifier can be found seen using:

<<echo=TRUE>>=
inspect(classifier[[1]])
@
