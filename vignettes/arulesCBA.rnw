...
\code{
> classifier <- CBA(dataset, className) \\
> classifier <- CBA(dataset, className, confidence = 0.95, verbose = TRUE)
}

These are only valid calls to the \textit{CBA} function if the following conditions are met:

\begin{itemize}
\item{The object \code{dataset} is a data.frame whose columns are all factors, or an \pkg{arules} transaction object}
\item{The object \code{className} is a character vector of size 1 whose value is the name of one of the columns of \code{dataset}. In other words, \code{className} should be an item from the list \code{colnames(dataset)}}
\item{The column in \code{dateset} whose name is \code{className} has no missing values}
\end{itemize}
...
The first step in classifying the data set is loading the data and discretizing the columns.
For this example, the Iris test data from the \pkg{datasets} will be used. The Iris dataset consists of 150 observations of 5 variables: sepal length, sepal width, pedal length, pedal width, and species for flowers. There are three species in the data set, and there are 50 instances of each species.

The data needs to be discretized because the CBA algorithm will convert the data set to \pkg{arules} transaction data, which can only be resolved from discrete or factor data. Refer to the \pkg{arules} documentation for more information on transaction data.
...
Note that the function \code{discretize} comes from \pkg{arules}, and the argument \code{categories = 9} refers to the number of categories which will be made. Once the continuous variables have been discretized, the class can be post-pended to the data.frame as follows:

<<echo=TRUE>>=
iris.disc$Species <- iris$Species
@

After this data preprocessing, the data should look as follows:
...
In order to ascertain the quality of the predicted classes, a confusion matrix will be built using the \pkg{caret} library:
